...
public class WillDoListActivity extends BaseActivity implements WillDoListContract.View {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        willDoListPresenter.getWillDoList(proTypeId,start,limit);
        setClient();
        setGestureListener();
    ...
    public void setWillDoList(WillDoList willDoList) {
        if (willDoList.getTotalCounts() == 0 && beanList.size() == 0) {
            if (recyclerView != null) {
                recyclerView.setVisibility(View.GONE);
                llNoContent.setVisibility(View.VISIBLE);
            }
        } else if (willDoList.getTotalCounts() == 0 && beanList.size() != 0) {
            if (recyclerView != null) {
                recyclerView.complete();
                recyclerView.onNoMore();
            }
        } else if (willDoList.getTotalCounts() != 0) {
        ...
        if (willDoList.getTotalCounts() < 20) {
            if (recyclerView != null) {
